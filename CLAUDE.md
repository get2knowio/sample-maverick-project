...
The repo uses a **tag-based baseline** (`baseline/main`) as the source of truth for clean state.

- **`scripts/reset-repo.sh [--force]`** — hard-resets `main` to `baseline/main`, deletes all other branches (local + remote), force-pushes, and runs `git clean -fdx`.
- **`scripts/test-setup.sh`** — sources reset-repo.sh, copies `.env` from the maverick project, cleans stale workspaces, and runs `maverick init --force`. After init, it copies `maverick.test.yaml` over the generated `maverick.yaml` to preserve the curated multi-provider config.
...
| File | Purpose |
|------|---------|
| `maverick.yaml` | Generated by `maverick init`; gets overwritten on every test run |
| `maverick.test.yaml` | Curated multi-provider config; copied over `maverick.yaml` after init |
| `maverick-copilot.yaml` | Copilot-specific config used by `test-flight-plan-e2e-copilot.sh` |
